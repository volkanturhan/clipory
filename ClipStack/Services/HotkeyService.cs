...
///
/// Like <see cref="ClipboardMonitor"/>, it owns an invisible message-only
/// window because Windows delivers the <c>WM_HOTKEY</c> notification as a
/// window message.
/// </summary>
public sealed class HotkeyService : IDisposable
{
    ...
    {
        var parameters = new HwndSourceParameters("ClipStackHotkey")
        {
        ...
        if (!RegisterHotKey(_source.Handle, HotkeyId, modifiers, VirtualKeyV))
            throw new Win32Exception(Marshal.GetLastWin32Error(),
                "Could not register the Ctrl+Shift+V hotkey; another app may already own it.");
    }
