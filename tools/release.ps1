# Builds a complete clipory release — the portable self-contained exe and the
# Windows installer — both named with the version read from clipory.csproj.
#
#   .\tools\release.ps1                   -> artifacts land in dist\release
#   .\tools\release.ps1 -OutDir <folder>  -> also copied into <folder>
#
# The -OutDir hook lets the outer workspace collect the build into builds\clipory
...
Write-Output "Building clipory v$version"
...
$portable = Join-Path $releaseDir "clipory-v$version.exe"
$setup    = Join-Path $releaseDir "clipory-setup-v$version.exe"
Copy-Item (Join-Path $selfContainedDir 'clipory.exe') $portable -Force
Copy-Item (Join-Path $root "dist\installer\clipory-setup-v$version.exe") $setup -Force
